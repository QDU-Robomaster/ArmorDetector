...
void ArmorDetector<CameraInfoV>::SetConfig(const Config& cfg)
{
  ...
  decoded_count_ = 0U;
  nms_count_ = 0U;
  semantic_kept_count_ = 0U;
  pnp_success_count_ = 0U;
  refined_count_ = 0U;
  refine_attempt_count_ = 0U;
  refine_fail_bbox_oob_count_ = 0U;
  refine_fail_roi_empty_count_ = 0U;
  refine_fail_lightbar_zero_count_ = 0U;
  refine_fail_lightbar_one_count_ = 0U;
  refine_fail_pair_distance_count_ = 0U;
  discarded_count_ = 0U;
  semantic_discard_count_ = 0U;
  type_discard_count_ = 0U;
  max_objectness_ = 0.0;
  model_ready_ = false;
  audit_every_frame_ = false;
  audit_zero_frames_ = false;
  disable_traditional_refine_env_ = false;
  center_letterbox_env_ = false;
  yolo_letterbox_env_ = false;
  dump_refine_fails_env_ = false;
  dump_refine_fails_dir_.clear();
  dump_refine_fails_max_ = 12U;
  dump_refine_fails_count_ = 0U;

  audit_every_frame_ = parse_env_flag("ARMOR_DETECTOR_AUDIT_EVERY_FRAME");
  audit_zero_frames_ = parse_env_flag("ARMOR_DETECTOR_AUDIT_ZERO_FRAMES");
  disable_traditional_refine_env_ =
      parse_env_flag("XR_ARMOR_DETECTOR_DISABLE_TRADITIONAL_REFINE");
  center_letterbox_env_ = parse_env_flag("XR_ARMOR_DETECTOR_CENTER_LETTERBOX");
  yolo_letterbox_env_ = parse_env_flag("XR_ARMOR_DETECTOR_YOLO_LETTERBOX");
  dump_refine_fails_env_ =
      parse_env_flag("XR_ARMOR_DETECTOR_DUMP_REFINE_FAILS");
  if (dump_refine_fails_env_)
  {
    dump_refine_fails_max_ =
    ...
    if (const char* env = std::getenv("XR_ARMOR_DETECTOR_DUMP_REFINE_FAILS_DIR"))
    {
      if (env[0] != '\0')
      {
        dump_refine_fails_dir_ = env;
      }
    }
    if (dump_refine_fails_dir_.empty())
    {
      dump_refine_fails_dir_ = "/tmp/xr_armor_refine_fails";
    }
    std::error_code ec;
    std::filesystem::create_directories(dump_refine_fails_dir_, ec);
    XR_LOG_INFO(
        "ArmorDetector refine failure dump enabled: dir=%s max=%u create_ok=%d",
        dump_refine_fails_dir_.c_str(), dump_refine_fails_max_, ec ? 0 : 1);
  }
                                              ...
                                              const SyncedFrame& synced_frame)
{
  ...
  metrics_msg_.decoded_count = decoded_count_;
  metrics_msg_.nms_count = nms_count_;
  metrics_msg_.semantic_kept_count = semantic_kept_count_;
  metrics_msg_.armor_count = static_cast<uint32_t>(armors_msg_.results.size());
  metrics_msg_.pnp_success_count = pnp_success_count_;
  metrics_msg_.refined_count = refined_count_;
  metrics_msg_.discarded_count = discarded_count_;
  metrics_msg_.semantic_discard_count = semantic_discard_count_;
  metrics_msg_.type_discard_count = type_discard_count_;
  metrics_msg_.max_objectness = max_objectness_;
  ...
  else if (audit_zero_frames_ && metrics_msg_.armor_count == 0U)
  {
    XR_LOG_WARN(
  ...
  else if ((frame_index_ % detail::metrics_log_period) == 0U)
  {
    XR_LOG_INFO(
        ...
        metrics_msg_.refined_count, refine_attempt_count_,
        refine_fail_bbox_oob_count_, refine_fail_roi_empty_count_,
        refine_fail_lightbar_zero_count_, refine_fail_lightbar_one_count_,
        refine_fail_pair_distance_count_, metrics_msg_.semantic_discard_count,
        metrics_msg_.type_discard_count, metrics_msg_.discarded_count,
                                   ...
                                   cv::Mat* binary_debug)
{
  ...
  const bool centered_letterbox = center_letterbox_env_ || yolo_letterbox_env_;
  ...
  const cv::Scalar letterbox_fill =
      yolo_letterbox_env_ ? cv::Scalar(114, 114, 114) : cv::Scalar(0, 0, 0);
                                         ...
                                         const cv::Mat& output, const cv::Mat& bgr_img)
{
  ...
  for (int row = 0; row < output.rows; ++row)
  {
    ...
    if (score > max_objectness_)
    {
      max_objectness_ = score;
    }
  ...
  decoded_count_ = static_cast<uint32_t>(detections.size());
  ...
  nms_count_ = static_cast<uint32_t>(indices.size());
  ...
  for (const int index : indices)
  {
    ...
    {
      ++discarded_count_;
      ++semantic_discard_count_;
      continue;
    }
    ++semantic_kept_count_;

    if (cfg_.yolo.use_traditional_refine && !disable_traditional_refine_env_)
    {
...
{
  auto* self = const_cast<ArmorDetector<CameraInfoV>*>(this);
  ++self->refine_attempt_count_;
  ...
  {
    ++self->refine_fail_bbox_oob_count_;
    return false;
  }
  ...
  if (armor_roi.empty())
  {
    ++self->refine_fail_roi_empty_count_;
    return false;
  }
  ...
  {
    ++self->refine_fail_pair_distance_count_;
...
{
  auto* self = const_cast<ArmorDetector<CameraInfoV>*>(this);
  if (!self->dump_refine_fails_env_ || armor_roi.empty() || binary_img.empty())
  {
    return;
  }
  if (self->dump_refine_fails_count_ >= self->dump_refine_fails_max_)
  {
    return;
  }

  const uint32_t dump_index = self->dump_refine_fails_count_++;
  std::error_code ec;
  std::filesystem::create_directories(self->dump_refine_fails_dir_, ec);
  if (ec)
  {
    XR_LOG_WARN("ArmorDetector failed to create refine dump dir: %s",
                self->dump_refine_fails_dir_.c_str());
    return;
  }

  std::ostringstream base_name;
  base_name << self->dump_refine_fails_dir_ << "/frame" << std::setw(6)
            << std::setfill('0') << self->frame_index_ << "_ts"
            << self->latest_timestamp_us_ << "_dump" << std::setw(2)
            << dump_index << "_" << reason;
  ...
  meta << "reason=" << reason << '\n';
  meta << "frame_index=" << self->frame_index_ << '\n';
  meta << "timestamp_us=" << self->latest_timestamp_us_ << '\n';
  meta << "threshold=" << self->cfg_.traditional.threshold << '\n';
