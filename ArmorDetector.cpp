...
ArmorNumber number_from_yolo_id(int number_id)
{
  switch (number_id)
  {
    case 0:
...
std::string armor_type_to_string(ArmorType type)
{
...
cv::Scalar color_to_scalar(ArmorColor color)
{
  switch (color)
  {
...
void draw_label_chip(cv::Mat& canvas, const std::string& text, const cv::Point& origin,
...
LibXR::Transform<double> make_pose(const cv::Mat& rvec, const cv::Mat& tvec)
{
  cv::Mat rotation_cv;
...
std::vector<ArmorDetector::CandidateArmor> ArmorDetector::Detect(const cv::Mat& raw_img,
                                                                 cv::Mat* binary_debug)
{
  refined_count_ = 0U;
  discarded_count_ = 0U;
  ...
  ov::Tensor input_tensor(ov::element::u8,
                          {1, YOLO_INPUT_SIZE, YOLO_INPUT_SIZE, 3}, input.data);
...
std::vector<ArmorDetector::CandidateArmor> ArmorDetector::Parse(
...
bool ArmorDetector::RefineArmorCorners(CandidateArmor& armor, const cv::Mat& bgr_img) const
{
...
std::vector<ArmorDetector::Lightbar> ArmorDetector::DetectLightbars(
...
bool ArmorDetector::ValidateLightbar(const Lightbar& lightbar) const
{
...
bool ArmorDetector::ValidateArmorGeometry(const CandidateArmor& armor) const
{
...
bool ArmorDetector::ValidateArmorType(const CandidateArmor& armor) const
{
...
bool ArmorDetector::HasContainedLightbar(const Lightbar& left, const Lightbar& right,
...
void ArmorDetector::UpdateGeometryMetrics(CandidateArmor& armor) const
{
...
ArmorType ArmorDetector::InferArmorType(const CandidateArmor& armor) const
{
  if (armor.ratio > 3.0)
  {
...
cv::Point2f ArmorDetector::GetNormalizedCenter(const cv::Mat& bgr_img,
...
ArmorColor ArmorDetector::GetContourColor(const cv::Mat& bgr_img,
...
void ArmorDetector::FillResultMessage(const std::vector<CandidateArmor>& armors,
...
void ArmorDetector::ShowDebugPreview(const cv::Mat& bgr_img, const cv::Mat* binary_debug)
{
  try
  {
...
bool ArmorDetector::ShouldShowPreview()
{
