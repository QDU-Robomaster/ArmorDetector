...
cv::Mat ArmorDetector::PreprocessImage(const cv::Mat& bgr_img)
{
  std::array<cv::Mat, 3> ch;
  cv::split(bgr_img, ch.data());

  cv::Mat binary_source;
  if (detect_color_ == RED)
  {
    cv::subtract(ch[2], ch[0], binary_source);
  }
  else if (detect_color_ == BLUE)
  {
    cv::subtract(ch[0], ch[2], binary_source);
  }
  else
  {
    cv::cvtColor(bgr_img, binary_source, cv::COLOR_BGR2GRAY);
  }

  cv::threshold(binary_source, binary_source, binary_thres_, 255, cv::THRESH_BINARY);
                                             ...
                                             const cv::Mat& binary_img)
{
  ...
  for (size_t i = 0; i < contours.size(); ++i)
  {
    ...
    const cv::Scalar MEAN_BGR = cv::mean(ROI, mask);
...
std::vector<Armor> ArmorDetector::Detect(const cv::Mat& input)
{
  ...
  if (!armors_.empty() && classifier_)
  {
    ...
    if (armors_.empty() && !raw_armors.empty())
    {
      XR_LOG_WARN(
          "Classifier removed all matched armors; using simulation fallback labels.");
      armors_ = raw_armors;
...
void ArmorDetector::ImageCallback(cv::Mat* img_msg)
{
  ...
  for (const auto& armor : ARMORS)
  {
    ...
    {
      XR_LOG_WARN(
          "PnP failed! type=%d pts=[(%.1f,%.1f),(%.1f,%.1f),(%.1f,%.1f),(%.1f,%.1f)]",
          static_cast<int>(armor.type), armor.left_light.bottom.x, armor.left_light.bottom.y,
          armor.left_light.top.x, armor.left_light.top.y, armor.right_light.top.x,
          armor.right_light.top.y, armor.right_light.bottom.x,
